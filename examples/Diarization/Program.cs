...
// This examples shows how to use Whisper.net to create a transcription from an
// audio file having multiple channels at 16Khz sample rate. Each channel is
// represented by a speaker. Based on the time of the segments, the speaker is
// identified, and the text is assigned to the speaker.

// We declare three variables which we will use later, ggmlType, modelFileName
// and inputFileName
...
// This section creates the whisperFactory object which is used to create the
// processor object.
using var whisperFactory = WhisperFactory.FromPath("ggml-base.bin");

// This section creates the processor object which is used to process the audio
// file, it uses language `auto` to detect the language of the audio file.
using var
...
await foreach (var result in processor.ProcessAsync(samples)) {
  ...
  // Iterate in the wave data to calculate total energy in each channel, and
  // find the channel with the maximum energy.
