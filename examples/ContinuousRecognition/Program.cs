...
var builder =
    whisperFactory.CreateBuilder().WithProbabilities().WithLanguage("en");
...
var partialResults = new List<(List<SegmentData> segments, TimeSpan startTime,
                               TimeSpan endTime)>();
var buffer = new byte[waveParser.SampleRate / 1000 * maxProcessingTimeMs * 2 *
                      waveParser.Channels];

var bufferSize = waveParser.SampleRate / 1000 * minProcessingTimeMs * 2 *
                 waveParser.Channels;
...
for (var i = 0; i < bytesRead;)
{
  ...
  samples[currentSampleIndex++] =
      sampleSum / (float)waveParser.Channels / 32768.0f;
}
...
while (currentSampleIndex < waveParser.SamplesCount)
{
  ...
  foreach (var partial in partialResults)
  {
    ...
    //  segments\n-----------");
    indexPartial++;
    // If one segment is identified. E.g. "My fellow Americans" from second 0 to
    // second 3 => we remove that part from the samples, adding the text to the
    // prompt and continue processing the rest of the samples.
  }

  // If the total current processing time is reaching max processing time => we
      ...
      maxProcessingTimeMs)
  {
    ...
    currentProcessedStartTime = currentProcessedStartTime.Add(
        TimeSpan.FromMilliseconds(maxProcessingTimeMs / 2));
