...
public sealed class WhisperProcessor : IAsyncDisposable, IDisposable
{
  ...
  // Id is used to identify the current instance when calling the callbacks from
  // C++
      ...
      [EnumeratorCancellation] CancellationToken cancellationToken = default)
  {
    ...
    try
    {
      ...
      var whisperTask =
          ProcessInternalAsync(samples, cancellationToken)
              .ContinueWith(
  ...
  private WhisperFullParams GetWhisperParams()
  {
    ...
    var whisperParams =
        Marshal.PtrToStructure<WhisperFullParams>(whisperParamsRef);
    ...
    unsafe
    {
      ...
      delegate *unmanaged[Cdecl]<IntPtr, IntPtr, IntPtr, byte>
          onEncoderBeginDelegate = &OnEncoderBeginStatic;
      whisperParams.OnEncoderBegin = (IntPtr)onEncoderBeginDelegate;

      delegate *unmanaged[Cdecl]<IntPtr, byte> onWhisperAbortDelegate =
...
#else
    // For netframework, we don't have `UnmanagedCallersOnlyAttribute` so we
    // need to use a delegate wrapped with a GC handle
    var onNewSegmentDelegate =
        new WhisperNewSegmentCallback(OnNewSegmentStatic);
    var gcHandle = GCHandle.Alloc(onNewSegmentDelegate);
    gcHandles.Add(gcHandle);
    whisperParams.OnNewSegment =
        Marshal.GetFunctionPointerForDelegate(onNewSegmentDelegate);

    var onEncoderBeginDelegate =
        new WhisperEncoderBeginCallback(OnEncoderBeginStatic);
    gcHandle = GCHandle.Alloc(onEncoderBeginDelegate);
    gcHandles.Add(gcHandle);
    whisperParams.OnEncoderBegin =
        Marshal.GetFunctionPointerForDelegate(onEncoderBeginDelegate);
    ...
    if (options.OnProgressHandlers.Count > 0)
    {
      ...
      gcHandles.Add(gcHandle);
      whisperParams.OnProgressCallback =
          Marshal.GetFunctionPointerForDelegate(onProgressDelegate);
      whisperParams.OnProgressCallbackUserData = myIntPtrId;
    }
#endif
  ...
  private static byte OnWhisperAbortStatic(IntPtr userData)
  {
    ...
    var shouldCancel =
        processor.options.WhisperAbortEventHandler?.Invoke() ?? false;
  ...
  private void OnNewSegment(IntPtr state)
  {
    ...
    while (segmentIndex < segments)
    {
      ...
      var t0 = TimeSpan.FromMilliseconds(
          NativeMethods.whisper_full_get_segment_t0_from_state(state,
                                                               segmentIndex) *
          10);
      var textAnsi = StringFromNativeUtf8(
          NativeMethods.whisper_full_get_segment_text_from_state(state,
                                                                 segmentIndex));
      ...
      var numberOfTokens =
          NativeMethods.whisper_full_n_tokens_from_state(state, segmentIndex);
