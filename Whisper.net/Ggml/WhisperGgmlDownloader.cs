...
public static class WhisperGgmlDownloader {
  ...
  /// </summary>
  /// <param name="type">The type of the model which needs to be
  /// downloaded.</param> <param name="quantization">The quantization of the
  /// model.</param> <param name="cancellationToken">A cancellation token used
  /// to cancell the request to huggingface.</param> <exception
  /// cref="ArgumentOutOfRangeException"></exception>
  public static async Task<Stream> GetGgmlModelAsync(
    ...
    var url =
        $"https://huggingface.co/sandrohanea/whisper.net/resolve/v2/{subdirectory}/{modelName}.bin";

    var request = new HttpRequestMessage(HttpMethod.Get, url);
    var response = await httpClient.Value.SendAsync(
  ...
  /// </summary>
  /// <param name="type">The type of the model which needs to be
  /// downloaded.</param> <param name="cancellationToken">A cancellation token
  /// used to stop the request to huggingface.</param> <remarks> Needs to be
  /// extracted on in the same directory as the ggml model, also ggml model
  /// needs to be loaded using file path, not stream.
  /// </remarks>
