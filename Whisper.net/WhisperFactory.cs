...
/// <summary>
/// A factory for creating <seealso cref="WhisperProcessorBuilder"/> used to
...
public sealed class WhisperFactory : IDisposable
{
  ...
  /// <summary>
  /// Creates a factory that uses the ggml model from a path in order to create
  /// <seealso cref="WhisperProcessorBuilder"/>.
  /// </summary>
  /// <param name="path">The path to the model.</param>
  /// <param name="delayInitialization">A value indicating if the model should
  /// be loaded right away or during the first <see cref="CreateBuilder"/>
  /// call.</param> <param name="libraryPath">The path to the library</param>
  /// <param name="bypassLoading">Bypass loading the library. Use this if you've
  /// already loaded the library through other means.</param> <param
  /// name="useGpu">A value indicating if the model should be loaded on the GPU
  /// or CPU (if runtime with GPU support is installed)</param> <returns>An
  /// instance to the same builder.</returns> <remarks> If you don't know where
  /// to find a ggml model, you can use <seealso
  /// cref="Ggml.WhisperGgmlDownloader"/> which is downloading a model from
  /// huggingface.co.
  /// </remarks>
  ...
  /// <summary>
  /// Creates a factory that uses the ggml model from a buffer in order to
  /// create <seealso cref="WhisperProcessorBuilder"/>.
  /// </summary>
  ...
  /// call.</param> <param name="libraryPath">The path to the library</param>
  /// <param name="bypassLoading">Bypass loading the library. Use this if you've
  /// already loaded the library though other means.</param> <param
  /// name="useGpu">A value indicating if the model should be loaded on the GPU
  /// or CPU (if runtime with GPU support is installed)</param> <returns>An
  /// instance to the same builder.</returns> <remarks> If you don't know where
  /// to find a ggml model, you can use <seealso
  ...
  /// <exception cref="ObjectDisposedException">Throws if the factory was
